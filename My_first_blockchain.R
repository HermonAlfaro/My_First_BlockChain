# Blocks
block_example <- list(index=1, timestamp="2018-01-05 17.00 MST", data ="some data", 
				previous_hash=0, proof=9, new_hash=NULL)
...
# Find the next number that is divisable by 99 and divisible by proof-number of the last block
proof_of_work <- function(last_proof){
	proof <- last_proof +1

	# Increment the proof number until a number is found that is divisable by 99 and the proof of the previous block
...
# ADDING NEW BLOCKS
#A function that takes the previous block and normally some data(ir our case the data is
# a string indicanting which block in the chain it is)
...
# Before to start building my blockchain, It's necessary to start the chain somewhere
